...
1. **Environment check**
   - Run `scripts/check_env.sh <project-or-template-root>` when available.
   - If `pandoc` or TeX tooling is missing, use `scripts/install_user_tools.sh` only after permission for network/user-environment writes is available.
...
4. **Convert**
   - Convert Markdown to a LaTeX fragment with `scripts/convert_md_to_tex.sh`.
...
5. **Integrate and compile**
   ...
   - Compile with `scripts/compile_template.sh`.
